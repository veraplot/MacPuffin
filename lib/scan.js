...
export async function scanJunk(onProgress, shouldStop, home = HOME) {
  const items = [];
...
export async function scanApps(onProgress, shouldStop, dirs = APP_DIRS) {
  const found = [];
...
export async function homeMap(onProgress, shouldStop, root = HOME) {
  let entries;
  try {
