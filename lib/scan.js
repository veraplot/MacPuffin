 ...
 */
/**
 * `shouldStop` is awaited, not polled. A plain boolean predicate still works,
 * and a control's `checkpoint` additionally parks the walk here for as long as
 * the user holds it paused.
 */
async function walk(root, { onFile, onBundle, onDir, onSkip, onDenied, shouldStop, skipDirs, skipNames, maxDepth = 24 }) {
  ...
  while (stack.length) {
    ...
    try {
      ...
      // A folder macOS will not let us read is not the same as an empty one.
      // Silently skipping it makes the total wrong with no way for the user to
      // know, which for a disk tool is worse than reporting nothing.
      if (err.code === 'EACCES' || err.code === 'EPERM') onDenied?.(dir);
      continue;
    }
    onDir?.(dir);
...
export async function deepScan(root, opts = {}) {
  ...
  // A closure rather than one literal at the end, so the identical shape can be
  // produced mid-walk when the user pauses. A paused scan showing an empty
  // screen would look broken; this makes everything found so far readable
  // without ending the walk. Duplicates need the whole file list before they
  // mean anything, so a paused snapshot reports none rather than guessing.
  const build = (duplicates = []) => ({
    root,
    files,
    totalBytes,
    scannedAt: now,
  ...
  await walk(root, {
    ...
    onDenied: (d) => { deniedCount += 1; if (denied.length < 20) denied.push(d); },
    onDir: (d) => {
      current = d;
      const now2 = Date.now();
...
async function findDuplicates(bySize, shouldStop, maxHash = 4000) {
  ...
  const hashes = await pool(targets, 12, async (t) => {
    if (await shouldStop?.()) return null;
    return { ...t, hash: await fingerprint(t.path) };
  });
...
export async function dirSize(dir, shouldStop) {
  let total = 0;
  let files = 0;
  let newest = 0;
  const stack = [dir];
  while (stack.length) {
    // One folder can hold a scan for minutes, so the gate belongs inside this
    // loop too — otherwise Pause appears to do nothing on the Cleanup, Apps and
    // Storage screens, which measure whole trees per item.
    if (await shouldStop?.()) return { size: total, files, newest, partial: true };
    const d = stack.pop();
    let entries;
    try {
...
export async function scanJunk(onProgress, shouldStop, home = HOME, onSnapshot) {
  const items = [];
  const build = () => {
    const sorted = [...items].sort((a, b) => b.size - a.size);
    return {
      items: sorted,
      total: sorted.reduce((s, i) => s + i.size, 0),
      reclaimable: sorted.filter((i) => i.safety === 'safe').reduce((s, i) => s + i.size, 0),
      scannedAt: Date.now(),
    };
  };
  onSnapshot?.(build);
  ...
  await pool(JUNK_TARGETS, 6, async (t) => {
    ...
    const { size, files, newest } = await dirSize(full, shouldStop);
...
export async function scanApps(onProgress, shouldStop, dirs = APP_DIRS, onSnapshot) {
  const found = [];
  const measured = [];
  const build = () => {
    ...
    return {
      apps: sorted,
      total: sorted.reduce((s, a) => s + a.size, 0),
      count: sorted.length,
      scannedAt: Date.now(),
    };
  };
  ...
  await pool(found, 6, async (app, i) => {
    if (await shouldStop?.()) return null;
    onProgress?.({ current: app.path });
    const { size, files } = await dirSize(app.path, shouldStop);
    let installed = 0;
    let lastUsed = 0;
    try {
    ...
    const m = meta[i] || {};
    // Collected as they finish rather than at the end, so a pause can report
    // the apps already measured.
    measured.push({ ...app, size, files, lastUsed: m.lastUsed || lastUsed, installed, version: m.version || '' });
    return null;
  });
...
export async function homeMap(onProgress, shouldStop, root = HOME, onSnapshot) {
  const measured = [];
  const build = () => {
    const sorted = [...measured].sort((a, b) => b.size - a.size);
    return { items: sorted, total: sorted.reduce((s, i) => s + i.size, 0), scannedAt: Date.now() };
  };
  onSnapshot?.(build);
  ...
  await pool(dirs, 4, async (d) => {
    if (await shouldStop?.()) return null;
    onProgress?.({ current: d });
    const { size, files } = await dirSize(d, shouldStop);
...
/**
 * Measure a list of paths and return the largest files among them.
 *
 * Separate from the Spotlight query so it can be tested on a machine whose
 * index is empty — a fresh CI runner has no large files to find, which would
 * otherwise leave this untested precisely where it matters.
 */
export async function statLargest(paths, limit = 200) {
  const stats = await pool(paths, 16, async (p) => {
    try {
      ...
      return null;   // vanished between the query and the stat
    }
  });
...
export async function spotlightLarge(minBytes = 1024 * 1024 * 1024, limit = 200) {
  ...
  return statLargest(paths, limit);
}
