...
/** MACPUFFIN_DOCKER_BIN points at a Docker installed somewhere unusual — and
 *  lets the tests reproduce a machine with no Docker at all. */
async function dockerBin() {
  const override = process.env.MACPUFFIN_DOCKER_BIN;
  if (override) {
    try {
...
export async function scanDocker(onProgress) {
  ...
  const images = parseDockerImages(raw);
  const df = await sh(bin, ['system', 'df', '--format', '{{json .}}'], { timeout: 15000 });
  const reclaimable = parseDockerReclaimable(df);
...
/**
 * Turn `docker image ls --format '{{json .}}'` output into image records.
 * Kept separate from the process call so the parsing — the part that actually
 * breaks when Docker changes its output — is testable on a machine with no
 * Docker installed, such as a CI runner.
 */
export function parseDockerImages(raw) {
  const images = [];
  for (const line of String(raw || '').split('\n').filter((l) => l.trim())) {
    ...
    try {
      ...
      continue;   // a warning line, or a format we do not recognise
    }
    if (!j.ID) continue;
    const untagged = j.Repository === '<none>' || j.Tag === '<none>';
    images.push({
      id: j.ID,
...
/** Pull the reclaimable figure for images out of `docker system df` output. */
export function parseDockerReclaimable(raw) {
  for (const line of String(raw || '').split('\n').filter((l) => l.trim())) {
    try {
      const j = JSON.parse(line);
      if (j.Type === 'Images' && j.Reclaimable) return parseDockerSize(j.Reclaimable);
...
export async function scanDevTools(onProgress, shouldStop, onSnapshot) {
  // The three lanes land independently, so a pause reports whichever have
  // finished and leaves the rest empty rather than discarding all of them.
  const partial = { python: null, node: null, docker: null };
  onSnapshot?.(() => ({
    python: partial.python || { envs: [], total: 0 },
    node: partial.node || { items: [], total: 0 },
    docker: partial.docker || { available: false, images: [], total: 0, reclaimable: 0 },
    scannedAt: Date.now(),
  }));

  const [python, node, docker] = await Promise.all([
    scanPython(onProgress, shouldStop).then((r) => (partial.python = r)),
    scanNode(onProgress, shouldStop).then((r) => (partial.node = r)),
    scanDocker(onProgress).then((r) => (partial.docker = r)),
  ]);
